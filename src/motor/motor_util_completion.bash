...
_motor_util_completion()
{
    ...
    COMREPLY=()
    local words
    local base_words="-l --list -c --check-messages-version --no-list --list-names-only --list-path-only --list-devpath-only --list-serial-number-only --list-devnum-only --no-dfu-list --dfu-leave -n --names -p --paths -d --devpaths -s --serial_numbers set read --set-api --api --run-stats -v --version -u --user-space --allow-simulated --lock -h --help";
    case $subcommand in
