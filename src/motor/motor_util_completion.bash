    ...
    case $subcommand in
        set) words="--host_time --mode --current --position --velocity --torque --torque_dot --reserved --gpio state position_tuning current_tuning stepper_tuning voltage stepper_velocity read -h --help";
            case $last in
                ...
                --mode) words="open damped current position velocity torque impedance state current_tuning position_tuning voltage phase_lock stepper_tuning hardware_brake driver_enable driver_disable clear_faults fault sleep crash reset" ;;
            esac ;;
        read) words="--poll --ppoll --aread --frequency --statistics --read-write-statistics --text -s --timestamp-in-seconds -t --host-time-seconds --publish --csv -f -r --reconnect --bits -v --compute-velocity --timestamp_frequency -p --precision set -h --help";
            case $last in
