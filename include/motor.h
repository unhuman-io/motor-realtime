...
enum ModeDesired {OPEN, DAMPED, CURRENT, POSITION, TORQUE, IMPEDANCE, VELOCITY, CURRENT_TUNING, POSITION_TUNING, VOLTAGE, PHASE_LOCK, RESET=255};
 ...
 public:
    UserSpaceMotor(std::string dev_path, uint8_t ep_num = 2) { 
       ...
       // struct udev_device *dev = udev_device_new_from_subsystem_sysname(udev, "usb", sysname);
        ...
        motor_txt_ = 0;//new USBFile(dev_path, 1);
