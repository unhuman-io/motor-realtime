...
class MotorManager {
 public:
    ...
    void set_command_position_tuning(TuningMode tuning_mode, 
         double amplitude, double frequency, double bias);
    void set_command_stepper_velocity(const std::vector<float> &current,  const std::vector<float> &velocity, double voltage = 0, StepperMode mode = StepperMode::STEPPER_CURRENT);
